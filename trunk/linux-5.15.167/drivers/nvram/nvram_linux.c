...
int fake_mt_mtd_read_nm_wifi(char *name, loff_t from, size_t len, u_char *buf){
	...
	if(strcmp(name,MTD_NVRAM_NAME)==0){
	memcpy(buf, fake_mtd_buffer+from, len);
	}
	return -1;
}
int fake_mt_mtd_write_nm_wifi(char *name, loff_t to, size_t len, const u_char *buf){
	printk("fake_mt_mtd_write_nm\n");
        ...
        if(strcmp(name,MTD_NVRAM_NAME)==0){
            memcpy( fake_mtd_buffer+to, buf,len);
        }
        return -1;
}
...
static const struct proc_ops nvram_ver_seq_fops = {
  .proc_open = nvram_ver_seq_open,
  .proc_read = seq_read,
