...
#define GET_MAX_PATH(_pchip_cap, _band_idx, _tx_rx) \
		(_pchip_cap->mcs_nss.max_path[_band_idx][_tx_rx])
...
s_int32 net_ad_dpd_prek(
	struct test_wlan_info *winfos,
	u_char op);
/* padavan: net_ad_thread_stop_tx 定义在 glue/osal/{jedi,gen4m}/net_adaption_*.c，
 * 但上游头文件里漏了声明，service/test_engine.c 直接调用 -> implicit declaration。*/
